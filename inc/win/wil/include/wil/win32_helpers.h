...
#pragma endregion
    ...
    /** Retrieves the fully qualified path for the file containing the specified xmodule loaded
    by a given process. Note GetModuleFileNameExW is a macro.*/
    template <typename string_type, size_t initialBufferLength = 128>
    HRESULT GetModuleFileNameExW(_In_opt_ HANDLE process, _In_opt_ HMODULE xmodule, string_type& path)
    {
             ...
             lengthWithNull = (wistd::min)(lengthWithNull * 2, maxExtendedPathLengthWithNull))
        {
            ...
            if (process != nullptr)
            {
                ...
                copiedCount = ::GetModuleFileNameExW(process, xmodule, maker.buffer(), static_cast<DWORD>(lengthWithNull));
            ...
            else
            {
                ...
                copiedCount = ::GetModuleFileNameW(xmodule, maker.buffer(), static_cast<DWORD>(lengthWithNull));
                copyFailed = (0 == copiedCount);
    ...
    /** Retrieves the fully qualified path for the file that contains the specified xmodule.
    The xmodule must have been loaded by the current process. The path returned will use the
    same format that was specified when the xmodule was loaded. Therefore, the path can be a
    long or short file name, and can have the prefix '\\?\'. */
    template <typename string_type, size_t initialBufferLength = 128>
    HRESULT GetModuleFileNameW(HMODULE xmodule, string_type& path)
    {
        return wil::GetModuleFileNameExW<string_type, initialBufferLength>(nullptr, xmodule, path);
    }
    ...
    template <typename string_type = wil::unique_cotaskmem_string>
    string_type GetModuleFileNameW(HMODULE xmodule)
    {
        string_type result;
    ...
    template <typename string_type = wil::unique_cotaskmem_string>
    string_type GetModuleFileNameExW(HANDLE process, HMODULE xmodule)
    {
        string_type result;
        THROW_IF_FAILED(wil::GetModuleFileNameExW(process, xmodule, result));
        return result;
    }
    ...
    the linker provides for every xmodule. This avoids the need for a global HINSTANCE variable
